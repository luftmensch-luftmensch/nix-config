...
let
  ...
  _notify = "${lib.getExe pkgs.libnotify} -t 800 -u low";
  _wl-mirror = "${lib.getExe pkgs.wl-mirror}";
  _fzf = "${lib.getExe pkgs.fzf} --reverse --height=50%  -i --pointer=\"▶\"   --info=inline --border=rounded --cycle";
in
